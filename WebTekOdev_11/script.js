document.addEventListener("DOMContentLoaded", function () {
  ...
  document.querySelectorAll('.slider a').forEach(link => {
    link.addEventListener('click', function (e) {
      ...
      document.querySelectorAll('.gezin').forEach(section => section.classList.remove('active'));
    ...
    fetch(`https://openlibrary.org/search.json?title=${encodeURIComponent(kitapIsmi)}`)
      ...
      .then(data => {
        ...
        fetch(`https://www.googleapis.com/books/v1/volumes?q=intitle:${encodeURIComponent(kitapIsmi)}`)
          ...
          .catch(() => alert("Kitap özeti alınamadı."));
      })
      .catch(err => {
        console.error(err);
...
if (studentNumber) {
  ...
  if (welcome) {
    ...
    welcome.textContent = "HOŞGELDİNİZ " + studentNumber;
  }
} else {
  // Hoşgeldiniz yazısını gizle
  const welcome = document.getElementById('welcome-message');
